...
def render_skos_markdown(ttl_path: Path, section_heading: str = "## Vocabulary") -> str:
    graph = Graph()
    ...
    if errors:
        raise ValueError("SKOS validation failed:\n- " + "\n- ".join(errors))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a markdown section from a SKOS Turtle file."
    )
    parser.add_argument(
        "--ttl",
        type=Path,
        required=True,
        help="Path to input SKOS Turtle file.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--section-heading",
        default="## Vocabulary",
        help="Top-level markdown heading for the generated section (default: '## Vocabulary').",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    rendered = render_skos_markdown(args.ttl, args.section_heading)
    if args.check:
        ...
        if existing != rendered:
            print(
                f"Markdown differs from generated output: {args.output}",
                file=sys.stderr,
            )
            return 1
        return 0
